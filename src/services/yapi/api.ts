...
import { YApiAuthService } from "./auth";
import type {
  ApiInterface,
  GetApiResponse,
  ProjectInfo,
...
export class YApiService {
  ...
  private authService: YApiAuthService | null = null;
  ...
  constructor(
    ...
    this.httpTimeoutMs = Number.isFinite(options.timeoutMs) ? Number(options.timeoutMs) : 5_000;
  ...
  /**
   * 设置authService实例，用于全局模式下获取登录态Cookie
   */
  setAuthService(authService: YApiAuthService | null): void {
  ...
  private async request<T>(
    ...
    try {
      ...
      const cookieHeader = this.authService ? await this.authService.getCookieHeaderWithLogin() : null;
   ...
   * 加载所有已配置项目的信息
   */
  async loadAllProjectInfo(): Promise<ProjectInfo[]> {
    ...
    try {
      // 并行加载所有项目的信息
      const projectInfos = await Promise.all(projectIds.map(id => this.getProjectInfo(id)));
      return projectInfos;
    } catch (error) {
  ...
  async searchApis(options: {
    ...
    try {
      ...
      if (projectKeyword && projectKeyword.trim().length > 0 && this.hasCookieAuth()) {
        ...
        projects = await this.loadAllProjectInfo();
