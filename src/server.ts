...
export class YapiMcpServer {
  ...
  constructor(
    ...
    if (this.authService) {
      this.yapiService.setAuthService(this.authService);
      this.logger.info("全局模式已启用。");
    }
    // 判断是否为stdio模式
    this.isStdioMode = process.env.NODE_ENV === "cli" || process.argv.includes("--stdio");
    
    this.logger.info(`YapiMcpServer初始化，日志级别: ${yapiLogLevel}`);
  ...
  private registerTools(): void {
    ...
    this.server.tool(
      "yapi_update_token",
      "全局模式：使用用户名/密码登录 YApi，刷新本地登录态 Cookie。",
      {
      ...
      async ({ forceLogin }) => {
        try {
          ...
          await this.authService.login(forceLogin);

          return {
            content: [
              {
                type: "text",
                text: `登录态刷新完成；已更新 Cookie。`,
              },
            ],
          };
        } catch (error) {
    ...
    this.server.tool(
      ...
      async () => {
        try {
          ...
          // 获取项目信息缓存
          const projectInfos = await this.yapiService.loadAllProjectInfo();
          ...
          // 构建项目信息列表
          const projectsList = projectInfos.map((info) => ({
            项目ID: info._id,
            项目名称: info.name,
            项目描述: info.desc || '无描述',
            基础路径: info.basepath || '/',
          ...
          return {
            content: [{
              type: "text",
              text: `已配置 ${projectInfos.length} 个YApi项目:\n\n${JSON.stringify(projectsList, null, 2)}`
            }],
          };
        } catch (error) {
